...
export function createModel(settings: Settings, inputDim: number) {
  const { layers, activation, learningRate, l2, problemType } = settings;
  ...
  model.compile({
    ...
    loss: problemType === 'classification' ? 'binaryCrossentropy' : 'meanSquaredError',
    metrics: problemType === 'classification' ? ['accuracy'] : ['mse']
  });
