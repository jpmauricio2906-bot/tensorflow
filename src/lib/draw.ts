...
export async function drawDecision({ ctx, width:w, height:h, model, settings }: DrawInputs){
  if (!model) return
...
export function drawPoints(ctx: CanvasRenderingContext2D, w: number, h: number, data:{xs:number[][], ys:number[]}, splitIndex:number, showTest:boolean){
  const rad = 3.5
  ctx.lineWidth = 1
